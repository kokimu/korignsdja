 ...
 * @return array
 */
function korignsdja_body_classes( $classes ) {
...
add_filter( 'body_class', 'korignsdja_body_classes' );

/**
 * Add a pingback url auto-discovery header for singularly identifiable articles.
 */
function korignsdja_pingback_header() {
	if ( is_singular() && pings_open() ) {
		echo '<link rel="pingback" href="', esc_url( get_bloginfo( 'pingback_url' ) ), '">';
	}
}
add_action( 'wp_head', 'korignsdja_pingback_header' );
 ...
 * @return array
 */
function korignsdja_filter_script_loader_tag( $tag, $handle ) {
...
add_filter( 'script_loader_tag', 'korignsdja_filter_script_loader_tag', 10, 2 );
 ...
 */
function korignsdja_get_preload_stylesheet_uri( $wp_styles, $handle ) {
 ...
 */
function korignsdja_add_body_style() {
	...
	// Preload content.css.
	$preloads['korignsdja-content'] = korignsdja_get_preload_stylesheet_uri( $wp_styles, 'korignsdja-content' );

	// Preload sidebar.css and widget.css.
	if ( is_active_sidebar( 'sidebar-1' ) ) {
		$preloads['korignsdja-sidebar'] = korignsdja_get_preload_stylesheet_uri( $wp_styles, 'korignsdja-sidebar' );
		$preloads['korignsdja-widgets'] = korignsdja_get_preload_stylesheet_uri( $wp_styles, 'korignsdja-widgets' );
	}

	// Preload comments.css.
	if ( ! post_password_required() && is_singular() && ( comments_open() || get_comments_number() ) ) {
		$preloads['korignsdja-comments'] = korignsdja_get_preload_stylesheet_uri( $wp_styles, 'korignsdja-comments' );
	}

	// Preload front-page.css.
	global $template;
	if ( 'front-page.php' === basename( $template ) ) {
		$preloads['korignsdja-front-page'] = korignsdja_get_preload_stylesheet_uri( $wp_styles, 'korignsdja-front-page' );
	}
...
add_action( 'wp_head', 'korignsdja_add_body_style' );
 ...
 */
function korignsdja_add_primary_menu_dropdown_symbol( $item_output, $item, $depth, $args ) {
...
add_filter( 'walker_nav_menu_start_el', 'korignsdja_add_primary_menu_dropdown_symbol', 10, 4 );
 ...
 */
function korignsdja_add_nav_menu_aria_current( $atts, $item ) {
	/*
...
add_filter( 'nav_menu_link_attributes', 'korignsdja_add_nav_menu_aria_current', 10, 2 );
add_filter( 'page_menu_link_attributes', 'korignsdja_add_nav_menu_aria_current', 10, 2 );
